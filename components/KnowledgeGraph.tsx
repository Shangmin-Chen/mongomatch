...
function getLayoutedElements(nodes: Node[], edges: Edge[]) {
  ...
  const layoutedNodes = nodes.map((node, idx) => {
    ...
    return {
      ...node,
      position: {
        x: nodeWithPosition && typeof nodeWithPosition.x === "number" ? nodeWithPosition.x - width / 2 : (idx % 3) * 260,
        y: nodeWithPosition && typeof nodeWithPosition.y === "number" ? nodeWithPosition.y - height / 2 : Math.floor(idx / 3) * 140,
      },
    };
  });
